        ...
        function vouchers() {
            ...
            xhttp.onreadystatechange = function() {
                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    let s12 = JSON.parse(this.responseText);
                    if (s12.response != "error") {
                        document.getElementById("checkoutcard").innerHTML = s12.checkoutbox;
                        s1.outerHTML = '<button type="button" class="btn btn-success" onclick="vouchers()">Đã áp dụng</button>';
                    } else {
                        alert("Mã giảm giá đã hết hạn hoặc không tồn tại")
                    }
                }
            }
            //cau hinh request
